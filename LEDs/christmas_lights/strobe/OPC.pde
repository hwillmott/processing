...
public class OPC
{
  ...
  void ledRing(int index, float x, float y, int numLEDs, float radius)
  {
    float angleStep = TWO_PI / numLEDs;
    float angle = 0;

    for (int i = 0; i < numLEDs; i++)
    {
      int posX = int(x + (cos(angle) * radius));
      int posY = int(y + (sin(angle) * radius));
      led(index + i, posX, posY);
      println("led at: " + posX + ", " + posY);
      angle += angleStep;
    }
  }
